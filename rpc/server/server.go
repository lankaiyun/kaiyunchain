...
func (s *Server) NewTx(ctx context.Context, req *pb.NewTxReq) (*pb.NewTxResp, error) {
	...
	// getWallet
	path := db.KeystoreDataPath + "/" + from
	w := wallet.LoadWallet(path, password, from)
	if w == nil {
	...
	tx := core.NewTransaction(val, uint64(time.Now().Unix()),
		common.BytesToAddress(fromByte), common.BytesToAddress(toByte), ecdsa.EncodePubKey(w.PubKey))
	txHash := tx.Hash()
